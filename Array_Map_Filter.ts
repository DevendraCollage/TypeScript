...
const EvenNum = numbers.filter((curElm: number) => curElm % 2 == 0)
console.log(EvenNum);

//? 2: Filtering numbers greater than 3

const GreaterNumb = numbers.filter((curElem: number) => curElem > 3)
...
const Names:string[] = ["Alice","Bob","Anna","Anderson","Alex"];
...
const Num2 = Num1.map((curVal:number) => curVal * 2);
console.log(Num1);
console.log(Num2);
...
function filterStringsByLength(arr: string[]): string[] {
    return arr.filter((str) => str.length > 15);
}

const inputArray: string[] = ["short", "medium length but have na 15 characters", "this string is longer than 15 characters"];const resultArray: string[] = filterStringsByLength(inputArray);
console.log(resultArray);

//? 2: GIven an array of strings, filter our the names the with the letter "A".
const Names3:string[] = ["Alice","Bob","Anna","Anderson","Alex"];
